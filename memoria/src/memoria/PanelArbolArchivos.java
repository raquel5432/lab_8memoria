...
public class PanelArbolArchivos extends JPanel {
    ...
    public PanelArbolArchivos(){
        ...
        arbolCarpetas.getSelectionModel()
        ...
        scrollArbol.setBorder(
            BorderFactory.createMatteBorder(0,0,0,1,Color.GRAY));
        scrollArbol.setHorizontalScrollBarPolicy(
            JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        JLabel etiquetaTitulo=new JLabel("  Carpetas");
        etiquetaTitulo.setFont(new Font(Font.SANS_SERIF,Font.BOLD,12));
        etiquetaTitulo.setBorder(BorderFactory.createCompoundBorder(
    ...
    private DefaultMutableTreeNode construirArbolDesdeRaices(){
        DefaultMutableTreeNode nodoRaiz=
            new DefaultMutableTreeNode("Este equipo");
        File[] unidades=File.listRoots();
        if(unidades!=null){
            for(File unidad:unidades){
                DefaultMutableTreeNode nodoUnidad=
                    new DefaultMutableTreeNode(
                        new NodoArchivoArbol(unidad));
                nodoUnidad.add(
                    new DefaultMutableTreeNode("Cargando..."));
    ...
    public void refrescarNodoSeleccionado(){
        ...
        if(nodo!=null){ expandirNodo(nodo); }
    }

    public DefaultMutableTreeNode getNodoSeleccionado(){
        return (DefaultMutableTreeNode)
            arbolCarpetas.getLastSelectedPathComponent();
    }
    ...
    public void agregarListenerSeleccion(TreeSelectionListener listener){
        arbolCarpetas.addTreeSelectionListener(listener);
    }

    public void agregarListenerExpansion(TreeExpansionListener listener){
        arbolCarpetas.addTreeExpansionListener(listener);
    }

    public static class NodoArchivoArbol {
        private final File archivo;
        public NodoArchivoArbol(File archivo){ this.archivo=archivo; }
